...
export function buildOccupiedMap(bookings = [], excludeBookingId = null) {
  const map = new Map()
  bookings.forEach(booking => {
    if (excludeBookingId && booking.id === excludeBookingId) return
    if (!booking.scheduled_date || !booking.scheduled_time) return
    const key = `${booking.scheduled_date}|${normalizeTime(booking.scheduled_time)}`
    map.set(key, (map.get(key) || 0) + 1)
  })
  return map
}

export function isSlotAvailable({ date, time, settings = {}, blackoutDates = [], slotBlocks = [], occupiedBookings = [], excludeBookingId = null }) {
  ...
  if (isSlotBlocked(date, normalized, slotBlocks, blackoutDates)) return false

  const occupied = buildOccupiedMap(occupiedBookings, excludeBookingId)
  return (occupied.get(`${date}|${normalized}`) || 0) < getSlotCapacity(settings)
}
